...
router.post('/run_ora/:jobID/:OS', async (req, res) => {
    ...
    keggOS = (keggOS === 'hsa') ? `RBR_WikiPathways_KEGG_hsa_pathways_compounds_R110.gmt` : `RBR_KEGG_${keggOS}_pathways_compounds_R110.gmt`;
    ...
    myPathKEGG_gmt = fs.existsSync(path.join(myPathKEGG_gmt, keggOS)) ?
        path.join(myPathKEGG_gmt, keggOS) : path.join(myPathKEGG_gmt, `RBR_WikiPathways_KEGG_hsa_pathways_compounds_R110.gmt`);

    // Get .gmt REACTOME path
    let reacOS = OS.replace(' ', '_');
    reacOS = (reacOS === 'Homo_sapiens' ) ? `RBR_WikiPathways_Reactome_Homo_sapiens_pathways_ChEBI_R89.gmt` : `RBR_Reactome_${reacOS}_pathways_ChEBI_R89.gmt`;
    ...
    myPathREAC_gmt = fs.existsSync(path.join(myPathREAC_gmt, reacOS)) ?
        path.join(myPathREAC_gmt, reacOS) : path.join(myPathREAC_gmt, `RBR_WikiPathways_Reactome_Homo_sapiens_pathways_ChEBI_R89.gmt`);
