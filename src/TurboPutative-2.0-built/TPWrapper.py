...
def main(args):
    ...
    for module in args.workflow:
        ...
        if module == '4': # TableMerger
            ...
            # Remove combinations of mz-rt generated by TableMerger (used in TurboOmics)
            if os.path.exists(os.path.join(args.workdir, '_mz_rt.tsv')): 
                logging.info("Remove pairs mz-rt created by TableMerger")
                config = configparser.ConfigParser()
                config.read(os.path.join(args.workdir, 'configFile.ini'))
                tm = pd.read_csv(os.path.join(args.workdir, config['TableMerger']['outfile']), sep='\t')
                mzrt = pd.read_csv(os.path.join(args.workdir, '_mz_rt.tsv'), sep='\t')
                tm = tm[[
                    np.logical_and(
                        mzrt.iloc[:, 0]-i<0.0001,
                        mzrt.iloc[:, 1]-j<0.0001
                        ).sum()>0
                    for i, j in zip(
                        tm[config["TableMerger"]["ms_column_mass"]],
                        tm[config["TableMerger"]["tm_column_rt"]]
                        )
                    ]]
                tm.to_csv(os.path.join(args.workdir, config['TableMerger']['outfile']), sep='\t', index=False)
